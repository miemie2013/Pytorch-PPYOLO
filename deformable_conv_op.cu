...
N, in_C H, W = x.shape
N, 2*kH*kW, out_H, out_W = offset.shape
N,   kH*kW, out_H, out_W = mask.shape
...
**/
    ...
    const int w_col = i % width_col;    # 卷积窗滑动的过程中，卷积的输出结果在新特征图的x坐标即w_col
    const int h_col = (i / width_col) % height_col;    # 卷积窗滑动的过程中，卷积的输出结果在新特征图的y坐标即h_col
    const int b_col = (i / width_col) / height_col % batch_size;    # 卷积窗滑动的过程中，滑动到了 原特征图的第b_col张图片
    const int c_im = (i / width_col / height_col) / batch_size;     # 卷积窗滑动的过程中，滑动到了 原特征图的第b_col张图片的第c_im个通道
    const int c_col = c_im * kernel_h * kernel_w;

    const int deformable_group_index = c_im / channel_per_deformable_group;

    const int h_in = h_col * stride_h - pad_h;    # 卷积窗滑动的过程中，卷积窗在原特征图的y坐标即h_in
    const int w_in = w_col * stride_w - pad_w;    # 卷积窗滑动的过程中，卷积窗在原特征图的x坐标即w_in

    T* data_col_ptr =
        data_col +
        ((c_col * batch_size + b_col) * height_col + h_col) * width_col + w_col;
    const T* data_im_ptr =   # 卷积窗滑动的过程中，滑动到了 原特征图的第b_col张图片的第c_im个通道。该通道的指针即data_im_ptr
    ...
    // 遍历卷积核的所有点（如果是3x3卷积核，即遍历9个点）
    for (int i = 0; i < kernel_h; ++i) {
      for (int j = 0; j < kernel_w; ++j) {
        const int data_offset_h_ptr =    // x偏移，相当于取 offset[bid, 2 * (i * kernel_w + j), h_col, w_col]
            ((2 * (i * kernel_w + j)) * height_col + h_col) * width_col + w_col;
        const int data_offset_w_ptr =    // y偏移，相当于取 offset[bid, 2 * (i * kernel_w + j) + 1, h_col, w_col]
            ((2 * (i * kernel_w + j) + 1) * height_col + h_col) * width_col + w_col;
        const int data_mask_hw_ptr =     // mask掩码，相当于取 mask[bid, (i * kernel_w + j), h_col, w_col]
            ((i * kernel_w + j) * height_col + h_col) * width_col + w_col;

        const T offset_h = data_offset_ptr[data_offset_h_ptr];   // x偏移
        const T offset_w = data_offset_ptr[data_offset_w_ptr];   // y偏移
        const T mask = data_mask_ptr[data_mask_hw_ptr];          // mask掩码
